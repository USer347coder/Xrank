    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-white/10 bg-black/25 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            {profile?.avatar_url && (
              <img src={profile.avatar_url} alt="" className="w-10 h-10 rounded-xl border border-white/10" />
            )}
            <div>
        ...
        {error && <div className="text-xs font-mono text-red-400 mb-3">{error}</div>}

        {cards.length === 0 && !loading ? (
          <div className="text-xs opacity-40 py-8 text-center">No snapshots for this username yet.</div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {cards.map((c) => (
              <div key={c.snapshot.id}>
                <div className="flex gap-2 mb-2">
                  <button onClick={() => setCompareA(c.snapshot.id)}
                    className={`px-2 py-1 rounded-lg border text-[10px] font-bold tracking-[0.14em] transition ${
                      compareA === c.snapshot.id ? 'border-white/30 bg-white/10' : 'border-white/10 bg-white/[0.03] opacity-60 hover:opacity-100'
                    }`}>SET A</button>
                  <button onClick={() => setCompareB(c.snapshot.id)}
                    className={`px-2 py-1 rounded-lg border text-[10px] font-bold tracking-[0.14em] transition ${
                      compareB === c.snapshot.id ? 'border-white/30 bg-white/10' : 'border-white/10 bg-white/[0.03] opacity-60 hover:opacity-100'
                    }`}>SET B</button>
                </div>
                <CardThumb card={c} />
              </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-black/25 p-5">
        ...
        {cardA && cardB ? (
          ...
          <div className="text-xs opacity-30 py-6 text-center">Choose two cards above to compare side-by-side.</div>
        )}
      </div>
    </div>
  );
