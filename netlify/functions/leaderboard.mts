...
export default async (req: Request) => {
  ...
  try {
    ...
    const prevWindowStart = prevMonthStart.toISOString();
    const prevWindowEnd = monthStart.toISOString();

    let windowLabel: 'previous_month' | 'current_month_so_far' = 'previous_month';
    let windowStart = prevWindowStart;
    let windowEnd = prevWindowEnd;

    // Launch safety: if there's no data yet for the previous month, fall back to the current month.
    let { shareableTotal, items } = await fetchWindow({ start: windowStart, end: windowEnd, limit });
    if (items.length === 0) {
      windowLabel = 'current_month_so_far';
      windowStart = monthStart.toISOString();
      windowEnd = now.toISOString();
      ({ shareableTotal, items } = await fetchWindow({ start: windowStart, end: windowEnd, limit }));
    }

    return jsonResponse({ window: { start: windowStart, end: windowEnd, label: windowLabel }, limit, total: shareableTotal, items });
  } catch (e: unknown) {
